...
from models import (
    HRZoneRequest, PaceZoneRequest, 
    HRZoneResponse, PaceZoneResponse, 
    ZoneResult, ErrorResponse
)
...
async def calculate_hr_zones(request: HRZoneRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Internal server error calculating HR zones"
        )
...
async def calculate_pace_zones(request: PaceZoneRequest):
    ...
    """
    try:
        logger.info(f"Calculating pace zones for {request.distance_meters}m in {request.time_seconds}s")
        ...
        raise HTTPException(
            status_code=500,
            detail="Internal server error calculating pace zones"
        )
