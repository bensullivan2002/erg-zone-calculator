...
def main():
    ...
    for key in zones:
        try:
            lower_bound = pace_calculator.calculate_lower_bound_time_per_500m(key, config_file_path)
            upper_bound = pace_calculator.calculate_upper_bound_time_per_500m(key, config_file_path)
    ...
    for key in zones:
        try:
            lower_bound = hr_calculator.calculate_lower_bound_HR(key, hr_config_file_path)
            upper_bound = hr_calculator.calculate_upper_bound_HR(key, hr_config_file_path)
            print(f"Zone: {key}, Lower Bound: {lower_bound}, Upper Bound: {upper_bound}")
        except Exception as e:
            print(f"Error calculating for zone {key}: {e}")
